...
func main() {
	...
	miner := []byte("KtoHe5H9jvViYgysX7ma8RW6LKLxks2GTdDpat5mGLnZxdV")
	var newBlock block.Blocks = block.Blocks{1, nil, nil, root, 1, 1577256099, nil, miner, nil, nil}
	newBlock.SetHash()
...
func AddBlock(b *block.Blocks) error {
	db := bg.New("blockchain.db")
	tx := db.NewTransaction()
	defer tx.Cancel()
	...
	//高度对下个高度
	err = tx.Set(miscellaneous.E64func(1), miscellaneous.E64func(2))
	if err != nil {
		fmt.Println(err)
		return err
	}
	tx.Del([]byte("height"))
	tx.Set([]byte("height"), miscellaneous.E64func(1))

	ar := types.BytesToAddress(b.Miner)

	MinerAccount(tx, ar.AddressToByte(), 1000000000000000000)
	return tx.Commit()
}
...
// 		setAccount(tx, Tx.From.AddressToByte(), Tx.To.AddressToByte(), Tx.Amount, Tx.Nonce)
...
// 	MinerAccount(tx, ar.AddressToByte(), 56110000000)
// 	return tx.Commit()
// }
